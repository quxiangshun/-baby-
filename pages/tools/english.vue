<template>
  <el-row v-if="isToolsMathPage" :gutter="16" class="p-10">
    <el-col
        ...
        :xl="4"
    >
      <el-card class="mb-5 cursor-pointer" style="padding: 5px 10px !important;" @click="navigateToTool(tool.url)">
        <div>{{ tool.name }}</div>
      </el-card>
    </el-col>
  </el-row>
  ...
  // 计算属性：判断是否为主页（路径为 '/'）
  const isToolsMathPage = computed(() => {
    return route.path === '/tools/english'
  })

  // 定义页面数据
  const toolList = ref([
    { name: "短句", icon: "", url: "/tools/english/phrase" },
  ])
